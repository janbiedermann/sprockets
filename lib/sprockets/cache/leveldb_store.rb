  ...
  class Cache
    ...
    class LeveldbStore
      ...
      def initialize(root = 'tmp/cache/sprockets_cache.leveldb', max_size = DEFAULT_MAX_SIZE, logger = self.class.default_logger)
        ...
        puts "Sprockets LevelDB Cache - max entries: #{@max_size}, current entries: #{@int_cache.size}, load time: #{(load_time * 1000).to_i}ms"
      end
      ...
      def gc!
        ...
        @logger.warn do
          ...
          "Sprockets LevelDB Cache [#{File.join(@root, 'sprockets_cache.gdbm')}] garbage collected " +
