...
module Sprockets
  class Cache
    ...
    class RawLeveldbStore
      ...
      #            (default: 1000).
      def initialize(root = 'tmp/cache/sprockets_cache.leveldb', max_size = DEFAULT_MAX_SIZE, logger = self.class.default_logger)
        start_time = Time.now
        @root = root
        @logger = logger
        @level = LevelDBNative::DB.new(root)
        at_exit do
          @level.close
        end
        @max_size = max_size
        ...
        puts "Sprockets RawLevelDB Cache - max entries: #{@max_size}, current entries: #{@size}, load time: #{(load_time * 1000).to_i}ms"
      end
      ...
      def get(key)
        value = nil
        str = @level.get(Marshal.dump(key))
        if str
      ...
      def gc!
        ...
        while @size > @gc_size
          @level.delete(Marshal.dump(k))
        end

        after_size = @size
        @logger.warn do
          time_diff = Time.now - start_time
          "Sprockets RawLevelDB Cache [#{File.join(@root, 'sprockets_cache.gdbm')}] garbage collected " +
